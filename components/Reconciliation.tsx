...
const Reconciliation: React.FC<ReconciliationProps> = ({
    ...
    return (
        <div className="space-y-5 pb-10 animate-fade-in">
            ...
            <div className="compact-note flex items-start gap-3 rounded-xl border border-indigo-100 bg-indigo-50/70 p-4 text-sm leading-relaxed text-indigo-950"><Info size={18} className="mt-0.5 shrink-0" /><p>每筆消費只屬於一個卡別與帳單月份。舊核銷紀錄沒有帳單月份時，請人工確認；「已核對」不代表已繳費。</p></div>
            <section className="w-fit max-w-full rounded-xl border border-slate-200 bg-white p-4">
                <form onSubmit={saveStatement} className="statement-fields">
                    ...
                    <div><label htmlFor="statement-amount" className="mb-2 block text-sm font-semibold text-slate-700">帳單總額 <span className="text-xs font-normal text-slate-500">依銀行帳單填寫</span></label><input id="statement-amount" type="number" step="0.01" required disabled={selectedBank === '-'} value={statementTotal} onChange={event => setStatementTotal(event.target.value)} placeholder="金額，無則填 0" className="w-full rounded-xl border border-slate-300 px-3 py-3 text-base font-number disabled:bg-slate-50" /></div>
            ...
            <section><h3 className="mb-3 text-lg font-semibold text-slate-800">各卡概況 · {selectedMonth}</h3><div className="tile-grid">{cardSummaries.map(card => <button type="button" key={card.bank} onClick={() => { setDetailBank(card.bank); setDetailKind(card.unassigned.length > 0 ? 'unassigned' : 'candidates'); }} className="rounded-2xl border border-slate-200 bg-white p-3 text-left transition-colors hover:border-indigo-300 hover:bg-indigo-50/30">
