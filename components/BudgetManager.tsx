                        ...
                        <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4">
                            ...
                            <div className="space-y-3">
                                ...
                                {cardRows.map(row => {
                                    ...
                                    return <div key={row.key} className="card-bill-row grid items-center gap-3 border-b border-slate-200 py-2 last:border-b-0">
                                        ...
                                        <label className="inline-flex min-h-11 cursor-pointer items-center gap-2 whitespace-nowrap text-xs text-slate-600"><input type="checkbox" disabled={!cardValue(row.key, row.amount).trim()} checked={!!cardValue(row.key, row.amount).trim() && (draft.paid[row.key] ?? !!row.isPaid)} onChange={event => setDraft(previous => ({ ...previous, paid: { ...previous.paid, [row.key]: event.target.checked } }))} className="h-5 w-5 shrink-0 rounded border-slate-300 text-indigo-600" />已繳費</label>
                                    </div>;
                                })}
                            </div>
                        </section>
