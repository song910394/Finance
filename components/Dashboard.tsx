    ...
    return (
        <div className="dashboard-layout space-y-4 pb-6 animate-fade-in">
            ...
            {(installmentSummary.groups.length > 0 || installmentSummary.unconfirmed.length > 0) && <section className="rounded-xl border border-slate-200 bg-white p-3">
                ...
                <div className="installment-cards mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2">{visibleInstallments.map((group, index) => <div key={group.id} data-bank-tone={index % 6} className="installment-card rounded-xl border border-slate-200 p-4">
                    <span aria-hidden="true" className="installment-flower">{['✿', '★', '❀', '✦', '♥', '✾'][index % 6]}</span>
                    ...
                    <div role="progressbar" aria-label={`${group.name} 已繳進度`} aria-valuemin={0} aria-valuemax={group.totalPeriods} aria-valuenow={group.reconciledPeriods} aria-valuetext={`已繳 ${group.reconciledPeriods} 期，共 ${group.totalPeriods} 期`} className="installment-progress"><span style={{ width: `${group.reconciledPeriods / group.totalPeriods * 100}%` }} /></div>
