...
import { backupMissingLeave, parseBackup } from '../utils/backup';
...
export default function Settings(props: SettingsProps) {
  ...
  const [missingLeave, setMissingLeave] = useState(false);
  ...
  const readBackup = async (file: File | undefined) => {
    ...
    try {
      ...
      setBackup(parseBackup(raw)); setMissingLeave(backupMissingLeave(raw)); setAction('import');
  ...
  return <div className="settings-layout">
    ...
    <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-4">
      ...
      <p className="text-sm text-slate-600">JSON 備份包含交易、卡片設定、月度帳務、入帳來源、薪資歷程及年假年度與休假明細。Excel 匯出僅包含交易明細。</p>
      <div className="flex flex-wrap gap-3">
    ...
    <section className="rounded-2xl border border-rose-200 bg-white p-4"><h3 className="font-bold text-rose-800">重設帳本</h3><p className="my-3 text-sm text-slate-600">清空交易、月度帳務、薪資、年假年度與休假明細，並重設記帳選項。連線恢復後，清空結果也會同步到雲端，請先匯出完整備份。</p><button type="button" className={buttonClass + ' text-rose-700'} onClick={() => setAction('reset')} disabled={busy || props.syncStatus === 'conflict'}><Trash2 size={18} />清空並重設帳本</button></section>
    <dialog ref={dialog} onClose={() => setAction(null)} aria-labelledby="settings-confirm-title" className="m-auto max-h-[calc(100dvh-2rem)] w-[calc(100%_-_2rem)] max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
      ...
      <div className="my-5 space-y-3 text-sm leading-relaxed text-slate-600">
        {action === 'import' && backup ? <><p>此備份包含 {backup.transactions.length} 筆交易、{backup.budgets.length} 個月份的帳務、{backup.salaryAdjustments.length} 筆薪資紀錄、{backup.leavePeriods.length} 個年假年度及 {backup.leaveRecords.length} 筆休假。</p>{missingLeave && <p className="font-bold text-rose-800">這份備份沒有年假欄位。若取代目前完整資料，現有年假也會被清空，不會合併不同版本。</p>}<p>確認後會整份取代目前帳本（包含年假），並依目前連線設定排入同步。系統會先保留完整原版本至「其他本機草稿」；備份失敗即停止取代。</p></> : action === 'reset' ? <p>目前交易、月度帳務、薪資、年假年度與休假明細將清空，記帳選項將重設；此結果會排入雲端同步。系統會先保留完整原版本，備份失敗即停止清空。</p> : <><p>{action === 'upload' ? '將使用目前完整帳本（含年假年度與休假明細）更新下方網址的雲端資料。若雲端有不同版本，可能被整份取代。' : '將讀取下方網址的完整帳本（包含年假）。所選版本沒有年假時，目前年假也會被清空，不會合併。有未同步的本機草稿時會先暫停，讓你選擇要保留的版本；取代前保留完整原版本，備份失敗即停止。'}</p><p className="break-all rounded-xl bg-slate-50 p-3 font-mono text-xs">{scriptUrl.trim()}</p></>}
      </div>
