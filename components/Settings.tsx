...
export default function Settings(props: SettingsProps) {
  ...
  return <div className="settings-layout">
    ...
    <section className="rounded-2xl border border-rose-200 bg-white p-4"><h3 className="font-bold text-rose-800">重設帳本</h3><p className="my-3 text-sm text-slate-600">清空交易、月度帳務與薪資紀錄，並重設記帳選項。連線恢復後，清空結果也會同步到雲端，請先匯出完整備份。</p><button type="button" className={buttonClass + ' text-rose-700'} onClick={() => setAction('reset')} disabled={busy || props.syncStatus === 'conflict'}><Trash2 size={18} />清空並重設帳本</button></section>
