...
(async () => {
  ...
  try {
    ...
    let failSave = false, lastPayload;
    await context.route(cloud + '**', route => {
      if (route.request().method() === 'POST') {
        lastPayload = JSON.parse(route.request().postData()).data;
        if (failSave) return route.fulfill({ status: 503, contentType: 'application/json', body: '{"success":false}' });
      }
    ...
    const saved = () => page.locator('header [data-sync-status="saved"]').waitFor();
    const snapshot = async () => {
      await saved();
      const url = await page.evaluate(() => localStorage.getItem('google_script_url'));
      const data = (await (await context.request.get(url)).json()).data ?? { transactions: [] };
      return { ...data, leavePeriods: data.leavePeriods ?? [], leaveRecords: data.leaveRecords ?? [] };
    };
    ...
    assert.deepEqual(await page.evaluate(() => Object.keys(localStorage).filter(key => key.startsWith('hs-finance:'))), []);
    await expect(page.getByText(/其他本機草稿/)).toHaveCount(0);
    ...
    assert.deepEqual(await snapshot(), backup);
    passed.push('重新載入、月份獨立、財務保留年假、錯誤匯入不變、舊版還原提示且不產生本機版本、實際下載JSON再還原、重設提示');
    ...
    failSave = true;
    await button('儲存休假').click();
    await expect(page.getByText('尚未上傳・請重試', { exact: true })).toBeVisible();
    let beforeRetry = lastPayload;
    ...
    const otherContext = await browser.newContext({ viewport: { width: 1440, height: 1000 }, serviceWorkers: 'block' });
    await otherContext.route('**/*', route => {
      if (new URL(route.request().url()).origin === origin) return route.continue();
      external.push(route.request().url()); return route.abort();
    });
    await otherContext.addInitScript(url => localStorage.setItem('google_script_url', url), cloud);
    const otherPage = await otherContext.newPage();
    otherPage.on('pageerror', error => errors.push(error.message));
    await otherPage.goto(origin + '/Finance/');
    await otherPage.locator('header [data-sync-status="saved"]').waitFor();
    await otherPage.getByRole('button', { name: '設定', exact: true }).click();
    await otherPage.getByLabel('預算金額', { exact: true }).fill('32000');
    await otherPage.getByRole('button', { name: '儲存預算', exact: true }).click();
    await otherPage.locator('header [data-sync-status="saved"]').waitFor();
    await button('新增休假').click();
    await page.getByLabel('用途', { exact: true }).selectOption('family');
    await page.getByLabel('休假日期', { exact: true }).fill('2026-09-21');
    await page.getByLabel('休假時數', { exact: true }).fill('0.5');
    await page.getByLabel('備註（選填）').fill('跨設備保留驗證');
    await button('儲存休假').click();
    await expect(page.getByRole('dialog')).toHaveCount(0);
    await saved();
    beforeRetry = await snapshot();
    assert.equal(beforeRetry.budget, 32000);
    const crossDeviceRecord = beforeRetry.leaveRecords.find(record => record.note === '跨設備保留驗證');
    assert.ok(crossDeviceRecord);
    await otherPage.getByLabel('預算金額', { exact: true }).fill('33000');
    await otherPage.getByRole('button', { name: '儲存預算', exact: true }).click();
    await otherPage.locator('header [data-sync-status="saved"]').waitFor();
    beforeRetry = await snapshot();
    assert.equal(beforeRetry.budget, 33000);
    assert.deepEqual(beforeRetry.leaveRecords.find(record => record.id === crossDeviceRecord.id), crossDeviceRecord);
    await expect(page.getByText(/其他本機草稿|本機與雲端有不同版本/)).toHaveCount(0);
    await expect(otherPage.getByText(/其他本機草稿|本機與雲端有不同版本/)).toHaveCount(0);
    await otherContext.close();
    await page.evaluate(() => window.dispatchEvent(new Event('focus')));
    passed.push('同步失敗明確提示／重試、兩個獨立設備先後儲存保留各自修改、不產生本機草稿');
